...
namespace HeiHGM::BMatching::runner::hypergraph_storage_system {
namespace {
using recursive_directory_iterator =
    std::filesystem::recursive_directory_iterator;
using HeiHGM::BMatching::app::app_io::HypergraphStorageSystemConfig;
} // namespace
HypergraphStorageSystem::HypergraphStorageSystem(absl::string_view directory)
    : _directory(directory) {
  reload();
}
HypergraphStorageSystemConfig HypergraphStorageSystem::config() const {
  return _config;
}
...
void HypergraphStorageSystem::reload() {
  ...
  if (!google::protobuf::TextFormat::ParseFromString(buffer.str(), &_config)) {
    std::cerr << "Parsing error in '" << _directory + "/storage.textproto"
              << "'" << std::endl;
    exit(1);
  }
...
void HypergraphStorageSystem::find_collection(
  ...
  if (collection_file.good()) {
    ...
    if (!google::protobuf::TextFormat::ParseFromString(buffer.str(),
                                                       &collection)) {
      std::cerr << "Parsing error in '"
                << path.string() + "/collection.textproto"
                << "'" << std::endl;
      exit(1);
    }
    ...
    *collection.mutable_root_path() = path.string();
    ...
    for (auto &hypergraph : *collection.mutable_hypergraphs()) {
      *hypergraph.mutable_file_path() =
          collection.root_path() + "/" + hypergraph.file_path();
    }
