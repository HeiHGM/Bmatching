...
def calc_perprof(
    ...
    for i, (prob, gr) in enumerate(grouped_by_problem):
        ...
        try:
            ...
            true_min = (
                gr.loc[gr["feas"] == True][perf_meas].max()[0]
                if maxvalue_good
                else gr.loc[gr["feas"] == True][perf_meas].min()[0]
            )
            # print(true_min)
            if inv_perf_meas == False:
                for i, row in gr.iterrows():
                    if row[solver_char[0]] not in results:
                        results[row[solver_char[0]]] = []
                    # print(row[perf_meas[0]])
                    results[row[solver_char[0]]] += [row[perf_meas[0]] / true_min]
            #   data.at[gr.loc[gr['feas'] == True].index,
            #          perf_meas[0]] = gr[perf_meas[0]] / true_min
            else:
                if i == 0:
    ...
    grouped_by_solver = results

    if tau_val is None:
        # Finding the unique tau values
        unique_taus = np.unique(
            np.sort([r for g in grouped_by_solver for r in grouped_by_solver[g]])
        )
    else:
    ...
    for n, tau in enumerate(unique_taus):
        for i, gr in enumerate(grouped_by_solver):
            if i == 0 and n == 0:
                print("Number of problems per solver: ", len(gr))
            array = np.array(grouped_by_solver[gr])
            solver_taus[i, n] = (
                np.sum(array >= tau) / len(grouped_by_problem)
                if maxvalue_good
                else np.sum(array <= tau) / len(grouped_by_problem)
            )
    ...
    return unique_taus, solver_taus, [s for s in grouped_by_solver], data
...
def draw_simple_pp(
    ...
    ax = fig.add_axes([0.125, 0.175, 0.8, 0.75])
    ...
    for n, solver in enumerate(solvers):
        ax.step(
            taus,
            solver_vals[n, :],
            label=escape_(solver),
            linestyle=styles[n % 4],
            alpha=0.7,
            marker=n,
            markersize=5,
        )
    ...
    ax.legend(
        loc=4 if legend_pos is None else legend_pos,
        bbox_to_anchor=None if legend_pos is None else (1.02, 0.5),
    )
    plt.xlim(1, taus.min() if max_is_good else taus.max())  # taus.max())
    ax.set_xlabel("$\\tau$")
